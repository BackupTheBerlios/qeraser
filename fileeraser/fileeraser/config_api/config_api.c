/*
 * $Id: config_api.c,v 1.1 2003/04/16 15:48:49 tobgle Exp $
 * 
		 ...
		 const char *value)
{
	...
	FILE *tmp_fp;
	char atemplate[] = "config_writeXXXXXX";
	...
	if (config_read(conf, section, option, tmp))
		if (!strcmp(tmp, value))
			return 1;
	...
	if ((section_offset = find_section(conf->fp, section)) == -1) {
		...
		tmp_fp = my_tmpfile(atemplate, "/tmp");
		if (tmp_fp == NULL) {
			if (conf->debug)
...
int config_get_num_options(config_t *conf, char *section) 
{
	...
	while (!feof(conf->fp)) {
		...
		if (is_an_option(line_tmp, NULL, 0,  NULL)) {
			count++;
		}
	}
